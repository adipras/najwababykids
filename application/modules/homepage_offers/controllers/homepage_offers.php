<?php if (!defined('BASEPATH')) exit('No direct script access allowed');
	class Homepage_offers extends MX_Controller {
		...
		function update($update_id)
        {
            ...
            $mysql_query = "SELECT ho.id, si.id as item_id, si.item_title
            				FROM homepage_offers ho
            					INNER JOIN homepage_blocks hb ON ho.block_id = hb.id
            					INNER JOIN store_items si ON ho.item_id = si.id
            				WHERE hb.id=$update_id";
            $data['query'] = $this->_custom_query($mysql_query);
            $data['num_rows'] = $data['query']->num_rows();

            $block_title = $this->homepage_blocks->_get_block_title($update_id);
            $data['headline'] = "Update Associated Offers ".$block_title;
